...
func MasonLight() {
	...
	for {
		...
		if online && !inTheOffice {
			inTheOffice = true
			//time.Sleep(30 * time.Second)
			fmt.Println("Came to office")
			sendData("cabinet_on")
		}
		if !online && inTheOffice {
			i++
			if i > 5 {
				inTheOffice = false
				fmt.Println("Left the office")
				sendData("cabinet_off")
			}
		}
...
func checkPing(config []IpConf) bool {
...
func sendData(s string) {
	http.Get("https://maker.ifttt.com/trigger/" + s + "/json/with/key/dcDZtLzGeg5CszTpCnxFtM")
}
